...
namespace dg
{
    ...
    namespace utils
    {
        ...
        class ThreadPool //FIXME : I suspect I'm vulnerable to race / spurious wakeups
        {
                class ThreadWorker
                {
                    ...
                    public:
                        ...
                        void operator()
                        (); //waits until notified, then it takes the first task and runs it
                };

                bool _stop_on_deallocation;
                bool _stopped; //did we ask to stop it ?
                std::queue<std::function<void()>>
                                               tasks; //list of tasks yet to perform
            ...
            public:
                ...
                explicit ThreadPool(unsigned n = std::thread::hardware_concurrency(),
                                    bool stop_on_deallocation = true);
                ...
                ThreadPool &operator=(ThreadPool &&) =
                        delete; //THAT SHALL NOT MOVE-AFFECT
        ...
        inline int ThreadPool::ThreadWorker::id_count = 1;

        inline ThreadPool::ThreadWorker::ThreadWorker(ThreadPool *pool) : _id(
                    id_count), pool(pool)
        {
            id_count++;
        }

        inline int ThreadPool::ThreadWorker::id() const
        {
            return _id;
        }
        ...
        inline ThreadPool::ThreadPool(unsigned n, bool stop_on_deallocation)
        ...
        inline void ThreadPool::join() //I suspect foulness
        {
        ...
        inline bool ThreadPool::stopped() const
        {
            return _stopped;
        }

        template<class Function, class ... Args>
        inline auto ThreadPool::enqueue(Function &&f,
                                        Args &&... args) -> std::future<decltype(f(args...))>
        {
            ...
            std::function<void()> wrapper
                    = [task_ptr]() //wrapper to match types of 'tasks'... ugly
            {
                (*task_ptr)();
            };
